...
public class QnaModifyServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		QnaDTO dto = dao.qnaModify(qbno);
		
		request.setAttribute("qnamodify", dto);
		RequestDispatcher rd = request.getRequestDispatcher("qna/qnamodify.jsp");
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		request.setCharacterEncoding("utf-8");
		 ...
		 String savepath = "upload"; //업로드 폴더
	      ...
	      dto.setQbno(qbno); //임의로 저장(글쓰기 화면엔 작성자(writer)가 없는데 테이블에 작성자가 있기때문에 null값이 넘어가면 안되니까)
